	...
	, m_parametric{ ANIME_NONE, ANIME_NONE }, m_parametricBlend(0.0f)
{
	...
	m_ModelInfoCb.m_ModelColor = { 1.0f, 1.0f, 1.0f };
}
MeshRenderer::~MeshRenderer()
{
	delete m_pBones;
...
void MeshRenderer::Start()
{
	...
	m_pModelInfoCb = std::make_shared<ConstantBuffer>();
	m_pModelInfoCb->Create(sizeof(ModelInfoCb));
...
void MeshRenderer::LateUpdate() {
	...
	float deg, rad;
	deg = 90.0f;
	rad = DirectX::XMConvertToRadians(deg);

	Quat q_src, q_rot1, q_rot2;
	q_src = Quat(vec0.x, vec0.y, vec0.z, 0.0f);
	q_rot1 = Quat(rad, axis);
	q_rot2 = q_rot1.getConjugate();
	...
	if (ObjectManager::FindObjectWithTag(TagName::MainCamera)->GetComponent<CameraEncounter>()) {
		m_Mat[1] = ObjectManager::FindObjectWithTag(TagName::MainCamera)->GetComponent<CameraEncounter>()->GetViewMatrix();
		m_Mat[2] = ObjectManager::FindObjectWithTag(TagName::MainCamera)->GetComponent<CameraEncounter>()->GetProjectionMatrix();
	}
	else
	if (ObjectManager::FindObjectWithTag(TagName::MainCamera)->GetComponent<CameraBattle>()) {
		m_Mat[1] = ObjectManager::FindObjectWithTag(TagName::MainCamera)->GetComponent<CameraBattle>()->GetViewMatrix();
		m_Mat[2] = ObjectManager::FindObjectWithTag(TagName::MainCamera)->GetComponent<CameraBattle>()->GetProjectionMatrix();
	}
}
...
{
	Step(1.0f / 60);
	ObjectManager::FindObjectWithTag(TagName::MainLight)->GetComponent<DirectionLight>()->SetLight();
	m_pVS->Bind();
	m_pPS->Bind();
	m_pWVP->Write(m_Mat);
	m_pWVP->BindVS(0);
	m_pModelInfoCb->Write(&m_ModelInfoCb);
	m_pModelInfoCb->BindVS(2);
	for (unsigned int i = 0; i < m_meshNum; ++i)
	{
		//UpdateBoneMatrix(i);
...
void MeshRenderer::SetColor(DirectX::XMFLOAT3 _color)
{
	// 色をセットする
	m_ModelInfoCb.m_ModelColor = _color;
}